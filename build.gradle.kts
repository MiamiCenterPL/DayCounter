...
val syncAssets = tasks.register<Copy>("syncAssets") {
    ...
    // IMPORTANT: Protect templates/configs from being overwritten
    exclude("manifest.json")
    exclude("majami.consumedurability.mixins.json")
...
val syncPlugin = tasks.register<Copy>("syncPlugin") {
    group = "hytale"
    description = "Copies this plugin jar into run/mods."
...
afterEvaluate {
    ...
    if (targetTask != null) {
        targetTask.dependsOn(syncPlugin)
        targetTask.finalizedBy(syncAssets)
        logger.lifecycle("✅ specific task '${targetTask.name}' hooked for asset auto-sync.")
        logger.lifecycle("syncPlugin attached to '${targetTask.name}' (mods/ loading).")
    } else {
        logger.warn("⚠️ Could not find 'runServer' or 'server' task to hook auto-sync into.")
    }
}

tasks.matching { it.name == "runServer" && it is JavaExec }.configureEach {
    // Remove project classes/resources from classpath so the server only loads mods/.
    doFirst {
        val exec = this as JavaExec
        val filtered = exec.classpath.filterNot { file ->
            val path = file.absolutePath.replace('\\', '/').lowercase()
            path.contains("/build/classes/") || path.contains("/build/resources/")
        }
        exec.classpath = files(filtered)
    }
}